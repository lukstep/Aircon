...
void Client::packetRecvCallback(void* arg, [[maybe_unused]] struct udp_pcb* udp,
                                struct pbuf* buffer, const ip_addr_t* addres,
                                uint16_t port) {
    ...
    if (ip_addr_cmp(addres, &ntp_ptr->serverIp) && port == ntp_ptr->PORT &&
        buffer->tot_len == ntp::Client::NTP_MSG_LEN && mode == 0x4 &&
        stratum != 0) {
        ntp_ptr->parseResponse(buffer);
    } else {
        logging::ERROR() << "Invalid NTP response\n";
    }
    pbuf_free(buffer);
}

void Client::serverDnsFoundCallback([[maybe_unused]] const char* hostname,
                                    const ip_addr_t* addres, void* arg) {
    Client* client = static_cast<Client*>(arg);
    if (addres) {
        client->setAddres(*addres);
        logging::INFO() << "NTP server IP addres " << ipaddr_ntoa(addres)
                        << "\n";
    } else {
        logging::ERROR() << "DNS request failed\n";
    }
...
bool Client::sendRequest() {
    ...
    if (buffer != nullptr) {
        ...
        logging::ERROR() << "pbuf allocation failed!\n";
        status = false;
    }
    cyw43_arch_lwip_end();
    return status;
...
bool Client::sendDnsRequest() {
    cyw43_arch_lwip_begin();
    int32_t err =
        dns_gethostbyname(SERVER_URL, &serverIp, serverDnsFoundCallback, this);
    cyw43_arch_lwip_end();
